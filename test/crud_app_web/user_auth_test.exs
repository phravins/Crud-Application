defmodule CrudAppWeb.UserAuthTest do
  use CrudAppWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias CrudApp.Accounts
  alias CrudApp.Accounts.Scope
  alias CrudAppWeb.UserAuth

  import CrudApp.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, CrudAppWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      CrudAppWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        CrudAppWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
